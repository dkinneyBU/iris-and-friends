...
RADIUS = 3  # neighborhood radius on the blackboard (try 3, then 5)
...
class Blackboard:
    ...
    def neighborhood_idxs(self, center, radius):
        n = len(self.slots)
        return [ (center + k) % n for k in range(-radius, radius + 1) ]

    def local_instant_majority(self, center, radius):
        idxs = self.neighborhood_idxs(center, radius)
        bits = [self.slots[i] for i in idxs]
        c = Counter(bits)
        return c.most_common(1)[0][0]

    def local_majority(self, center, radius):
        # Temporal majority, but only over a local neighborhood
        idxs = self.neighborhood_idxs(center, radius)
        if not self.history:
            bits = [self.slots[i] for i in idxs]
        else:
            ...
            for snap in self.history:
                bits.extend(snap[i] for i in idxs)
        c = Counter(bits)
        return c.most_common(1)[0][0]
...
class Agent:
    ...
    def update(self, bb: Blackboard, ctx: str):
        ...
        center = random.randrange(len(bb.slots))

        # Read: align to temporal majority (with noise)
        maj = bb.local_majority(center, RADIUS)
        read_prob = 0.30 if ctx == "A" else 0.15
        ...
        inst = bb.local_instant_majority(center, RADIUS)

        # weighted compatibility: persistence matters more than momentary vibe
        score = 0
        if behavior == maj:
            score += 1
        if behavior == inst:
        ...
        if random.random() < (WRITE_PROB * coupling):
            idxs = bb.neighborhood_idxs(center, RADIUS)
...
def run():
    ...
    for _ in range(STEPS):
        ...
        def patch_diversity(bb):
            # how many local-majority regions are 1 vs 0 across the board
            locals_ = [bb.local_instant_majority(i, RADIUS) for i in range(len(bb.slots))]
            c = Counter(locals_)
            p0 = c.get(0, 0) / len(locals_)
            p1 = c.get(1, 0) / len(locals_)
            return -sum(p*np.log2(p) for p in [p0, p1] if p > 0)

        patch_div_A.append(patch_diversity(bbA))
        patch_div_B.append(patch_diversity(bbB))
